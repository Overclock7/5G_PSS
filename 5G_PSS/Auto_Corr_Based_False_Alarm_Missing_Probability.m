% function [P_F,P_M] = Auto_Corr_Based_False_Alarm_Missing_Probability(SNR_dB)
...
for k = 1:length(threshold)
    ...
    for m = 1:N_ITER
        ...
        for l = 1:length(rx_pss)-(N_IFFT-2)
            ...
            corr_result = abs(sum(rx_pss(l+1:1:l+(N_IFFT/2-1)) .* rx_pss(l+(N_IFFT-1):-1:l+N_IFFT-(N_IFFT/2-1))));
