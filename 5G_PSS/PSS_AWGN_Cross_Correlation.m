...
%% For Comparison
tx_pss_1 = sqrt(N_IFFT)*ifft(ifftshift(pss_1),N_IFFT);
tx_pss_2 = sqrt(N_IFFT)*ifft(ifftshift(pss_2),N_IFFT);
...
[pss_0_corr_result,pss_0_corr_index] = xcorr(rx_pss_0,tx_pss_0,255);
...
%% Freq Domain
fft_rx_pss_0 = 1/sqrt(N_IFFT) * fftshift(fft(rx_pss_0,N_IFFT));
fft_rx_pss_0_cp = 1/sqrt(N_IFFT) * fftshift(fft(rx_pss_0_cp(N_CP+1:end),N_IFFT));

%% For Constellation
part_fft_rx_pss_0 = fft_rx_pss_0(65:191);
part_fft_rx_pss_0_cp = fft_rx_pss_0_cp(65:191);
...
subplot(311);
stem(pss_0_corr_index, abs(pss_0_corr_result));
title("TX and Rx signal Correlation");
ylim([0,140]);
ylabel("R[\tau]");
subplot(312);
stem(pss_1_corr_index, abs(pss_1_corr_result));
ylim([0,140]);
ylabel("R[\tau]");
subplot(313);
stem(pss_2_corr_index, abs(pss_2_corr_result));
ylim([0,140]);
ylabel("R[\tau]");
xlabel("\tau");
